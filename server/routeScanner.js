...
const NEST_METHOD_DECORATOR_RE = new RegExp(
...
function scanExpressFastify(content, filePath, out) {
	...
	for (const m of content.matchAll(EXPRESS_FASTIFY_RE)) {
...
function scanNest(content, filePath, out) {
	...
	for (const m of content.matchAll(NEST_METHOD_DECORATOR_RE)) {
		const [, method, subPath] = m
		const full =
			`${base}${subPath ? `/${subPath}` : ""}`.replace(/\/+/g, "/") || "/"
		out.push({
			method: method.toUpperCase(),
			path: full,
			file: filePath,
	...
	const requests = deduped
		...
		.map((r, i) => {
			...
			return {
				...
				headers: [],
				params: [],
				body: ["POST", "PUT", "PATCH"].includes(r.method)
					? { type: "json", content: "{}" }
					: { type: "none", content: "" },
				auth: { type: "none" },

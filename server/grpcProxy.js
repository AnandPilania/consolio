...
const sendToUi = (wss, tabId, payload) =>
	sendToType(wss, `grpc-proxy:${tabId}`, payload)
...
export function handleGrpcProxyConnection(ws, _wss, tabId) {
	ws.on("close", () => {
...
export function handleGrpcProxyMessage(_ws, wss, tabId, raw) {
	let msg
	try {
		msg = JSON.parse(raw)
	} catch {
	...
	if (msg.action === "loadProto") {
		try {
			...
			sendToUi(wss, tabId, {
				action: "schema",
				methods: methods.map(({ path, requestStream, responseStream }) => ({
					path,
					requestStream,
					responseStream,
				})),
	...
	if (msg.action === "call") {
		...
		let loaded
		try {
			loaded = loadProtoServices(msg.protoText)
		} catch (e) {
			sendToUi(wss, tabId, {
				action: "status",
				connected: false,
				error: `Proto error: ${e.message}`,
			})
			return
		}
		...
		if (!methodDef) {
			sendToUi(wss, tabId, {
				action: "status",
				connected: false,
				error: `Method not found in proto: ${msg.methodPath}`,
			})
			return
		}
		...
		if (methodDef.requestStream) {
			sendToUi(wss, tabId, {
				...
				connected: false,
				error:
					"Client-streaming and bidirectional methods are not supported yet — use a unary or server-streaming method.",
			})
			return
		}
		...
		if (!ServiceCtor) {
			sendToUi(wss, tabId, {
				action: "status",
				connected: false,
				error: `Service not found: ${svcPath}`,
			})
			return
		}
		...
		let requestObj = {}
		try {
			requestObj = msg.requestJson?.trim() ? JSON.parse(msg.requestJson) : {}
		} catch (e) {
			sendToUi(wss, tabId, {
				action: "status",
				connected: false,
				error: `Request JSON parse error: ${e.message}`,
			})
			return
		}
		...
		let client
		try {
			client = new ServiceCtor(msg.address, grpc.credentials.createInsecure())
		} catch (e) {
			sendToUi(wss, tabId, {
				action: "status",
				connected: false,

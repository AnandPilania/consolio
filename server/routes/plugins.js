...
export async function pluginRoutes(fastify, { storage }) {
    ...
    fastify.get("/api/plugins/ui", async () =>
        listPaneTabs(await loadEnabledPlugins(storage)),
    )

    fastify.post("/api/plugins/ui/render", async (req, reply) => {
        const { pane, plugin, id, context } = req.body || {}
        if (
            !["request", "response"].includes(pane) ||
        ...
        try {
            return await renderPaneTab(await loadEnabledPlugins(storage), {
                pane,
                plugin,
                id,
                context,
    ...
    fastify.post("/api/plugins/update", async (req, reply) => {
        const name = req.body?.name
        const bundled = listBundledPlugins().find((plugin) => plugin.name === name)
        if (!bundled)
            return reply.status(400).send({ error: "Unknown bundled plugin" })
        try {
            return await installBundledPlugin(storage, bundled.dir)
        } catch (e) {
            return reply
                .status(500)
                .send({ error: `npm update failed: ${e.message}` })
        }
    })
    ...
    fastify.post("/api/plugins", async (req, reply) => {
        ...
        try {
            ...
            return reply
                .status(500)
                .send({ error: `npm install failed: ${e.message}` })
        }
    })
    ...
    fastify.delete("/api/plugins/*", async (req, reply) => {
        ...
        try {
            ...
            return reply
                .status(500)
                .send({ error: `npm uninstall failed: ${e.message}` })
        }
    })

...
export async function environmentRoutes(fastify, { storage }) {
    ...
    fastify.put('/api/environments/:id', async (req, reply) => {
        ...
        return storage.saveEnvironment({ ...env, ...req.body, id: env.id });
    });

    fastify.delete('/api/environments/:id', async (req) => {
...
export async function configRoutes(fastify, { storage }) {
    fastify.get('/api/config', async () => {
        ...
        return { ...config, isProjectMode: storage.isProjectMode, projectPath: storage.consolioDir };
    });

    fastify.put('/api/config', async (req) => {
        const updated = { ...storage.getConfig(), ...req.body };

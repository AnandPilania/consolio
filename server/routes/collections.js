...
import { callProvider, suggestFixes } from "../ai-suggest.js"
import { diffCollections } from "../diffCollections.js"
import {
	applyProfile,
	createProfile,
	describeProfileImpact,
} from "../profiles.js"
...
export async function collectionRoutes(fastify, { storage }) {
	...
	fastify.put(
		"/api/collections/:id/profiles/:profileId",
		async (req, reply) => {
			const col = storage.getCollection(req.params.id)
			if (!col) return reply.status(404).send({ error: "Collection not found" })
			const idx = (col.profiles || []).findIndex(
	...
	fastify.delete(
		...
		async (req, reply) => {
			const col = storage.getCollection(req.params.id)
			if (!col) return reply.status(404).send({ error: "Collection not found" })
			col.profiles = (col.profiles || []).filter(
				(p) => p.id !== req.params.profileId,
			)
			storage.saveCollection(col)
			return { deleted: true }
	...
	fastify.get(
		"/api/collections/:id/profiles/:profileId/impact",
		async (req, reply) => {
			const col = storage.getCollection(req.params.id)
			if (!col) return reply.status(404).send({ error: "Collection not found" })
			const profile = (col.profiles || []).find(
				(p) => p.id === req.params.profileId,
			)
			if (!profile)
				return reply.status(404).send({ error: "Profile not found" })
	...
	fastify.get(
		"/api/collections/:id/profiles/:profileId/apply",
		async (req, reply) => {
			const col = storage.getCollection(req.params.id)
			if (!col) return reply.status(404).send({ error: "Collection not found" })
			const profile = (col.profiles || []).find(
	...
	fastify.post("/api/collections/:id/diff", async (req, reply) => {
		...
		if (!candidate || !Array.isArray(candidate.requests)) {
			return reply.status(400).send({
				error:
					"body.candidate (a collection-shaped object with a requests[] array) is required",
			})
		}
	...
	fastify.post(
		"/api/collections/:colId/requests/:reqId/ai-suggest",
		async (req, reply) => {
			const { provider, apiKey, model, baseUrl } = req.body || {}
			if (!provider)
				return reply.status(400).send({
					error:
						'provider is required (e.g. "anthropic", "openai", "azure-openai", "ollama", "openai-compatible")',
				})

			const col = storage.getCollection(req.params.colId)
			if (!col) return reply.status(404).send({ error: "Collection not found" })
			const request = (col.requests || []).find(
				(r) => r.id === req.params.reqId,
			)
			if (!request)
				return reply.status(404).send({ error: "Request not found" })

			const lastEntry = storage
				.getAllHistory()
				.find((h) => h.requestId === request.id)
	...
	fastify.get("/api/collections/:id/mcp-manifest", async (req, reply) => {
		...
		const registered = registerCollectionTools(previewServer, col, {
			storage,
		})
	...
	fastify.delete(
		"/api/collections/:colId/requests/:reqId",
		async (req, reply) => {
			const col = storage.getCollection(req.params.colId)
			if (!col) return reply.status(404).send({ error: "Collection not found" })
	...
	fastify.patch("/api/collections/:id/reorder", async (req, reply) => {
		...
		col.requests = order
			.map((entry) => {
				...
				if (!r) return null
				if (typeof entry === "object" && "folderId" in entry)
					r.folderId = entry.folderId
				return r
			})
			.filter(Boolean)
		storage.saveCollection(col)
	...
	fastify.put("/api/collections/:id/folders/:folderId", async (req, reply) => {
		const col = storage.getCollection(req.params.id)
		if (!col) return reply.status(404).send({ error: "Collection not found" })
		const idx = (col.folders || []).findIndex(
			(f) => f.id === req.params.folderId,
		)
	...
	fastify.delete(
		...
		async (req, reply) => {
			...
			col.requests = col.requests.map((r) =>

...
const sendToUi = (wss, tabId, payload) =>
	sendToType(wss, `sse-proxy:${tabId}`, payload)
...
export function handleSseProxyConnection(ws, _wss, tabId) {
	ws.on("close", () => {
...
export async function handleSseProxyMessage(_ws, wss, tabId, raw) {
	let msg
	try {
		msg = JSON.parse(raw)
	} catch {

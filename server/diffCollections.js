...
export function diffCollections(before, after) {
	const beforeByKey = new Map(
		(before.requests || []).map((r) => [requestKey(r), r]),
	)
	const afterByKey = new Map(
		(after.requests || []).map((r) => [requestKey(r), r]),
	)
	...
	for (const [key, oldReq] of beforeByKey) {
		...
		const oldParams = paramKeys(oldReq),
			newParams = paramKeys(newReq)
		const removedParams = [...oldParams].filter((p) => !newParams.has(p))
		if (removedParams.length)
			changes.push({ type: "params_removed", detail: removedParams })

		// Removed headers (excluding Content-Type, which routinely varies without being breaking).
		const oldHeaders = headerKeys(oldReq),
			newHeaders = headerKeys(newReq)
		const removedHeaders = [...oldHeaders].filter(
			(h) => h !== "content-type" && !newHeaders.has(h),
		)
		if (removedHeaders.length)
			changes.push({ type: "headers_removed", detail: removedHeaders })
		...
		if (oldAuthType !== newAuthType)
			changes.push({
				type: "auth_changed",
				detail: { from: oldAuthType, to: newAuthType },
			})
		...
		if (oldBodyType !== newBodyType)
			changes.push({
				type: "body_type_changed",
				detail: { from: oldBodyType, to: newBodyType },
			})

		if (changes.length) {
			changedEndpoints.push({
				key,
				name: newReq.name || oldReq.name,
	...
	const breaking =
		...
		changedEndpoints.some((c) =>
			c.changes.some((ch) => ch.type !== "headers_removed"),
		)

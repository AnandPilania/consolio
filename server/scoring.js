const CHECKS = [
	...
	},
	{
		...
		weight: 15,
		test: (r) => Boolean(r.auth?.type && r.auth.type !== "none"),
	...
	},
	{
		...
		test: (r) =>
			...
			r.tests.some((t) =>
				["status", "status_lt", "status_in"].includes(t.type),
			),
	...
	},
	{
		...
		weight: 10,
		test: (r) => Boolean(r.name?.trim() && r.name.trim() !== "New Request"),
	...
	},
	{
		...
		weight: 10,
		test: (r) => {
			const entries = [...(r.params || []), ...(r.headers || [])].filter(
				(p) => p.key,
			)
			if (entries.length === 0) return true // nothing to document
			return entries.every((p) => p.description?.trim())
		},
	...
	},
	{
		...
		test: (r) => {
			const suspicious =
				/(api[_-]?key|token|secret|password)\s*[:=]\s*[A-Za-z0-9_-]{8,}/i
			const haystack = [r.url, ...(r.headers || []).map((h) => h.value)]
				.filter(Boolean)
				.join(" ")
...
export function scoreCollection(collection) {
	...
	for (const r of scored) {
		for (const c of r.checks) {
			if (!c.passed) {
				failuresByCheck[c.id] = failuresByCheck[c.id] || {
					id: c.id,
					label: c.label,
					fix: c.fix,
					count: 0,
	...
	const topIssues = Object.values(failuresByCheck).sort(
		(a, b) => b.count - a.count,
	)

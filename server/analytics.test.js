...
test("buildAnalytics: empty history returns zeroed-out stats, not NaN/crash", async () => {
...
test("buildAnalytics: computes avg latency, p95, and error rate correctly", async () => {
	const entries = [
...
test("buildAnalytics: scopes to a single collection when collectionId is passed", async () => {
	const entries = [
...
test("buildAnalytics: per-request breakdown groups by requestId, sorted by volume desc", async () => {
	const entries = [
...
test("buildAnalytics: recentErrors captures failed calls with method/url/status", async () => {

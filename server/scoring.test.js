...
import { scoreCollection, scoreRequest } from "./scoring.js"

test("scoreRequest: a fully-empty default request fails most checks", async () => {
	const req = {
		id: "req_1",
		name: "New Request",
		headers: [],
...
test("scoreRequest: a well-documented request with auth + status test scores high", async () => {
	const req = {
		id: "req_2",
		name: "Get user by id",
...
test("scoreRequest: hardcoded-looking secret in a header is flagged unless templated", async () => {
	const bad = {
		id: "req_3",
		name: "x",
		headers: [
			{
				key: "Authorization",
				value: "api_key=sk_live_abcdef1234567890",
				enabled: true,
			},
		],
		tests: [],
	...
	const good = {
		id: "req_4",
		name: "x",
		headers: [{ key: "Authorization", value: "api_key={{SK}}", enabled: true }],
		tests: [],
		auth: { type: "none" },
	}
	assert.strictEqual(
		scoreRequest(bad).checks.find((c) => c.id === "no_hardcoded_secrets")
			.passed,
		false,
	)
	assert.strictEqual(
		scoreRequest(good).checks.find((c) => c.id === "no_hardcoded_secrets")
			.passed,
		true,
	)
})

test("scoreCollection: empty collection scores 0 with a clear reason, not NaN", async () => {
...
test("scoreCollection: averages request scores and rolls up top issues by frequency", async () => {
	const col = {
		id: "col_2",
		name: "Mixed",
		requests: [
	...
	assert.ok(
		result.topIssues.every(
			(issue, i) => i === 0 || issue.count <= result.topIssues[i - 1].count,
		),
	)
})

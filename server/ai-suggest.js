...
export async function suggestFixes(request, lastResponse, callModel) {
	if (typeof callModel !== "function") {
		throw new Error(
			"An AI provider call is required — suggestFixes never calls out on its own",
		)
	}
...
export async function callOpenAICompatible({ apiKey, model, prompt, baseUrl }) {
	if (!baseUrl)
		throw new Error(
			"baseUrl is required for an OpenAI-compatible provider (e.g. https://api.openai.com/v1, an Azure OpenAI deployment URL, or http://localhost:11434/v1 for Ollama)",
		)
	if (!model)
		throw new Error("model is required for an OpenAI-compatible provider")
	const { default: fetch } = await import("node-fetch")
...
export async function callOllama({
	...
	if (!model)
		throw new Error(
			'model is required for the Ollama provider (e.g. "llama3.1", "qwen2.5")',
		)
	const { default: fetch } = await import("node-fetch")
...
export async function callProvider(provider, options) {
	...
	if (!caller) {
		throw new Error(
			`Unknown AI provider "${provider}". Supported: ${Object.keys(PROVIDER_CALLERS).join(", ")}`,
		)
	}

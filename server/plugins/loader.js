...
function isNewerVersion(candidate, current) {
    ...
    if (
        !candidate ||
        !current ||
        !/^\d+(?:\.\d+){0,2}/.test(String(candidate)) ||
        !/^\d+(?:\.\d+){0,2}/.test(String(current))
    )
        return false
    return next.some(
        (part, index) =>
            part !== installed[index] &&
            part > installed[index] &&
            next.slice(0, index).every((value, i) => value === installed[i]),
    )
}
...
function ensurePluginsProject(dir) {
    ...
    if (!existsSync(pkgPath)) {
        writeFileSync(
            pkgPath,
            JSON.stringify(
                { name: "consolio-plugins", private: true, dependencies: {} },
                null,
                2,
            ),
        )
...
export function listInstalledPlugins(storage) {
    ...
    return Object.entries(pkg.dependencies || {}).map(([name, version]) => {
        ...
        const bundledPlugin = listBundledPlugins().find(
            (plugin) => plugin.name === name,
        )
        let installedPkg = { name, version }
        try {
            installedPkg = JSON.parse(readFileSync(installedPkgPath, "utf8"))
        } catch { }
        const installedVersion = installedPkg.version || version
        const bundledVersion = bundledPlugin?.version || null
        return {
            ...pluginMetadata({ ...bundledPlugin, ...installedPkg }, name),
            name,
            version: installedVersion,
            installedVersion,
            latestVersion: bundledVersion,
            updateAvailable: isNewerVersion(bundledVersion, installedVersion),
            enabled: manifest.enabled[name] !== false,
        }
    })
}
...
export async function installPlugin(storage, name) {
    ...
    await execFileAsync(
        NPM_CMD,
        ["install", "--prefix", dir, "--no-audit", "--no-fund", name],
...
export async function uninstallPlugin(storage, name) {
    ...
    await execFileAsync(
        NPM_CMD,
        ["uninstall", "--prefix", dir, "--no-audit", "--no-fund", name],
...
export async function loadEnabledPlugins(storage) {
    ...
    for (const installedPlugin of installed) {
        ...
        try {
            ...
            hooks.pluginMeta[name] = {
                ...pluginMetadata({ ...installedPlugin, ...pkg }, name),
                installedVersion: installedPlugin.installedVersion,
                latestVersion: installedPlugin.latestVersion,
                updateAvailable: installedPlugin.updateAvailable,
            }
            if (Array.isArray(plugin.requestHooks))
                hooks.requestHooks.push(...plugin.requestHooks)
            if (Array.isArray(plugin.responseHooks))
                hooks.responseHooks.push(...plugin.responseHooks)
            if (plugin.templateTags && typeof plugin.templateTags === "object")
                Object.assign(hooks.templateTags, plugin.templateTags)
...
export function listPaneTabs(hooks) {
    return {
        request: (hooks.paneTabs?.request || []).map(({ plugin, id, label }) => ({
            plugin,
            id,
            label,
            pluginInfo: hooks.pluginMeta?.[plugin],
        })),
        response: (hooks.paneTabs?.response || []).map(({ plugin, id, label }) => ({
            plugin,
            id,
            label,
            pluginInfo: hooks.pluginMeta?.[plugin],
...
export async function renderPaneTab(hooks, { pane, plugin, id, context }) {
    const tab = hooks.paneTabs?.[pane]?.find(
        (item) => item.plugin === plugin && item.id === id,
    )

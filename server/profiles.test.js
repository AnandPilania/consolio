...
test("createProfile: requires a name, validates mode, de-dupes and lowercases redacted headers", async () => {
	assert.throws(() => createProfile({ name: "" }), /name is required/)
	assert.throws(
		() => createProfile({ name: "x", mode: "nonsense" }),
		/mode must be/,
	)
	const p = createProfile({
		name: "Public",
		requestIds: ["r1", "r1"],
		redactHeaders: ["X-Admin-Token", "x-admin-token"],
	})
...
test("applyProfile: allowlist mode keeps only listed requests, drops unused folders", async () => {
...
test("applyProfile: blocklist mode excludes listed requests, keeps everything else", async () => {
	const col = sampleCollection()
	const profile = createProfile({
		name: "No Admin",
		mode: "blocklist",
...
test("applyProfile: redactHeaders/redactParams strip matching entries from every visible request", async () => {
	const col = sampleCollection()
	const profile = createProfile({
		name: "External",
		mode: "allowlist",
		requestIds: ["r1", "r3"],
		redactHeaders: ["X-Admin-Token"],
		redactParams: ["limit"],
	})
	const filtered = applyProfile(col, profile)
...
test("applyProfile: original collection object is never mutated", async () => {
	const col = sampleCollection()
...
test("describeProfileImpact: reports included/excluded counts and redaction lists", async () => {
	const col = sampleCollection()
	const profile = createProfile({
		name: "Public",
		mode: "allowlist",

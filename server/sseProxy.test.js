...
test("Multiple complete events, plus a trailing incomplete one carried over", async () => {
	const { events, remainder } = extractEvents(
		"data: hello\n\nevent: ping\ndata: pong\nid: 1\n\ndata: incompl",
	)
...
test("Multi-line data joins with \n; comment-only blocks produce no event", async () => {
...
test("CRLF line endings are normalized the same way", async () => {

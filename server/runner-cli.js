...
export async function runCollectionCli(collectionArg, options) {
	...
	const runOne = async (req) => {
		...
		const pass =
			!payload.error &&
			(hasTests
				? payload.testResults.every((t) => t.pass)
				: payload.status < 400)
		return {
			name: req.name || req.url || "Unnamed",
			pass,
	...
	for (let i = 0; i < reqs.length; i += concurrency) {
		...
		chunkResults.forEach((r, k) => {
			results[i + k] = r
		})
		if (options.bail && chunkResults.some((r) => !r.pass)) bailed = true
		if (!bailed && delay > 0 && i + concurrency < reqs.length)
	...
	if (reporter === "json") {
		...
		ran.forEach((r) => {
			...
			console.log(
				`${icon} ${r.name} ${chalk.dim(`(${r.status ?? "—"}, ${r.elapsed}ms)`)}`,
			)
		...
		console.log("")
		console.log(
			`${chalk.bold(ran.length)} run, ${chalk.green(`${passCount} passed`)}, ${chalk.red(`${failCount} failed`)}${skipped ? `, ${skipped} skipped` : ""}`,
		)
	}

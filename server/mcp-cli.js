...
function findCollection(storage, arg) {
	...
	return (
		cols.find((c) => c.id === arg) ||
		cols.find((c) => (c.name || "").toLowerCase() === arg.toLowerCase())
	)
}
...
export async function serveMcpCli(collectionArg, options) {
	...
	if (!col.requests?.length) {
		console.error(
			chalk.red(
				`Collection "${col.name}" has no requests — nothing to expose as MCP tools.`,
			),
		)
		return 2
	}
	...
	const secretVarNames = new Set(
		(env?.variables || []).filter((v) => v.secret).map((v) => v.key),
	)
	...
	const mcpServer = new McpServer({
		name: `consolio-${col.name}`,
		version: "1.0.0",
	})
	const registered = registerCollectionTools(mcpServer, col, {
		storage,
		baseEnvironment,
		secretVarNames,
	})
	...
	console.error(
		chalk.dim(
			`  ${registered.length} tool(s) registered${env ? ` (environment: ${env.name})` : ""}`,
		),
	)
...
export async function generateMcpManifest(collectionArg, options) {
	...
	if (options.out) {
		...
		writeFileSync(
			options.out,
			JSON.stringify(
				{ server: `consolio-${col.name}`, tools: manifest },
				null,
				2,
			),
		)
		console.log(
			chalk.green(
				`✔  Wrote ${manifest.length} tool manifest entries to ${options.out}`,
			),
		)
	} else {
		console.log(
			chalk.green(
				`✔  "${col.name}" would expose ${manifest.length} MCP tool(s):`,
			),
		)
		for (const m of manifest)
			console.log(`   ${chalk.cyan(m.name.padEnd(30))} ${m.request}`)
		console.log(
			chalk.dim(

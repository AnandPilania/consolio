...
function summarize(entries) {
    ...
    if (total === 0) {
        return { totalRequests: 0, avgLatencyMs: 0, p95LatencyMs: 0, errorRate: 0, errorCount: 0 };
    }
    const latencies = entries.map(e => e.response?.elapsed ?? 0).sort((a, b) => a - b);
    const errorCount = entries.filter(e => (e.response?.status ?? 0) >= 400).length;
...
export function buildAnalytics(entries, { collectionId = null } = {}) {
    const scoped = collectionId ? entries.filter(e => e.collectionId === collectionId) : entries;
    ...
    for (const e of scoped) {
        ...
        byRequest[key] = byRequest[key] || { requestId: key, requestName: e.requestName || e.request?.url || key, entries: [] };
        byRequest[key].entries.push(e);
    }
    const requestBreakdown = Object.values(byRequest)
        .map(({ requestId, requestName, entries }) => ({ requestId, requestName, ...summarize(entries) }))
    ...
    const recentErrors = scoped
        ...
        .slice(0, 20)
        .map(e => ({
            id: e.id, timestamp: e.timestamp, requestName: e.requestName || e.request?.url,

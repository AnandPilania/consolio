...
export function runTests(tests, response) {
	return (tests || []).map((t) => {
		try {
			...
			switch (t.type) {
				case "status":
					pass = response.status === parseInt(t.value, 10)
				...
				case "status_lt":
					...
					break
				case "status_in": {
					const codes = String(t.value)
						.split(",")
						.map((v) => parseInt(v.trim(), 10))
					pass = codes.includes(response.status)
					actual = String(response.status)
					break
				}
				case "has_header": {
					const k = t.value.toLowerCase()
					pass = !!response.headers?.[k]
					actual = response.headers?.[k] || "(missing)"
					break
				}
				case "header_equals": {
					...
					pass = actualVal === expected
					actual = actualVal ?? "(missing)"
					break
				}
				case "body_contains":
					pass = (response.body || "").includes(t.value)
					actual = pass ? "✓" : "✗"
					break
				...
				case "body_json_path": {
					try {
						...
						pass = actual === t.value || cur === t.value
					} catch {
						actual = "parse error"
						pass = false
					}
				...
				case "response_time":
					pass = response.elapsed <= parseInt(t.value, 10)
					actual = fmtTime(response.elapsed)
					break
				case "response_time_gt":
					...
					break
				case "body_not_empty":
					pass = !!response.body?.trim()
					actual = pass ? "has body" : "empty"
					break
				default:
					pass = false
					actual = "unknown type"
			...
			return { ...t, pass: false, actual: e.message, ran: true }
		}
	})
}

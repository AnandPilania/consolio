export function createProfile({
	...
	if (!["allowlist", "blocklist"].includes(mode))
		throw new Error('mode must be "allowlist" or "blocklist"')
	return {
...
function redactRequest(request, profile) {
	...
	return {
		...
		headers: (request.headers || []).filter(
			(h) => !stripHeaderSet.has((h.key || "").toLowerCase()),
		),
...
export function applyProfile(collection, profile) {
	...
	const folders = (collection.folders || []).filter((f) =>
		usedFolderIds.has(f.id),
	)

...
const sendToUi = (wss, tabId, payload) =>
	sendToType(wss, `sio-proxy:${tabId}`, payload)

export function handleSocketIoProxyConnection(ws, _wss, tabId) {
	ws.on("close", () => {
...
export function handleSocketIoProxyMessage(_ws, wss, tabId, raw) {
	let msg
	try {
		msg = JSON.parse(raw)
	} catch {

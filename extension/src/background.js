...
chrome.storage.local.get(["enabled", "port"], (data) => {
	...
	consolioPort = parseInt(data.port ?? 4242, 10)
	updateBadge()
})
...
chrome.storage.onChanged.addListener((changes) => {
	...
	if (changes.port !== undefined)
		consolioPort = parseInt(changes.port.newValue, 10)
})

function updateBadge() {
	chrome.action.setBadgeText({ text: isEnabled ? "ON" : "" })
	chrome.action.setBadgeBackgroundColor({
		color: isEnabled ? "#f59e0b" : "#555555",
	})
}
...
chrome.webRequest.onCompleted.addListener(
	(details) => {
		...
		if (
			details.url.includes(`localhost:${consolioPort}`) ||
			details.url.includes(`127.0.0.1:${consolioPort}`)
		)
			return
...
async function sendToConsolio(entry) {
	try {
		await self.fetch(
			`http://localhost:${consolioPort}/api/interceptor/capture`,
			{
				method: "POST",

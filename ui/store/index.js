        ...
        (set, get) => ({
            ...
            closeTab(id) {
                set((s) => {
                    ...
                    const activeTabId =
                        s.activeTabId === id
                            ? final[Math.max(0, s.tabs.findIndex((t) => t.id === id) - 1)]
                                ?.id || final[0].id
                            : s.activeTabId
            ...
            /* ── Request send ──────────────────────────────────────────────────── */
            async sendRequest() {
                const { getActiveTab, getEnvVars, activeEnvId, updateTab, showNotif } =
                    get()
                const tab = getActiveTab()
                if (!tab.url.trim()) {
                    showNotif("Enter a URL first", "error")
                    return
                }
                ...
                try {
                    const res = await apiFetch("/api/execute", {
                        ...
                        body: {
                            ...
                            collectionId: tab.activeReq?.colId || null,
                            requestId: tab.activeReq?.reqId || null,
                            requestName: tab.reqName || tab.name || null,
                        },
                    })
                    if (res.preScriptError)
                        showNotif(`Pre-script: ${res.preScriptError}`, "error")
                    if (res.postScriptError)
                        showNotif(`Post-script: ${res.postScriptError}`, "error")
                    updateTab(tab.id, {
                        response: res,
                        loading: false,
                        testResults: res.testResults || [],
                        preLogs: res.preLogs || [],
                        postLogs: res.postLogs || [],
                    })
                    if (!res.error)
                        apiFetch("/api/history?limit=30")
                            .then((h) => set({ history: h }))
                            .catch(() => { })
                    if (activeEnvId && res.environment) {
                        apiFetch("/api/environments")
                            .then((envs) => set({ environments: envs }))
                            .catch(() => { })
                    }
                } catch (e) {
                    updateTab(tab.id, {
                ...
                set((s) => ({
                    tabs: s.tabs.map((t) =>
                        t.id === tabId
                            ? {
                                ...t,
                                wsFrames: [...(t.wsFrames || []), frame].slice(-500),
                            }
                            : t,
                    ),
                }))
            ...
            connectWs(url) {
                ...
                const tab = getActiveTab()
                if (!url?.trim()) {
                    showNotif("Enter a WebSocket URL first", "error")
                    return
                }
                disconnectWs()
                const tabId = tab.id
                ...
                const sock = new WebSocket(
                    `${proto}://${window.location.host}/ws?type=${encodeURIComponent(`ws-proxy:${tabId}`)}`,
                )
                sock.onopen = () => {
                ...
                sock.onmessage = (ev) => {
                    ...
                    if (msg.action === "status") {
                        ...
                        if (msg.error) {
                            showNotif(`WebSocket: ${msg.error}`, "error")
                            appendWsFrame(tabId, {
                                id: uid(),
                                direction: "system",
                                timestamp: Date.now(),
                            ...
                            appendWsFrame(tabId, {
                                ...
                                direction: "system",
                                timestamp: Date.now(),
                                data: msg.connected ? "Connected" : "Disconnected",
                            })
            ...
            sendWsMessage(data) {
                ...
                if (sock && sock.readyState === WebSocket.OPEN)
                    sock.send(JSON.stringify({ action: "send", data }))
                ...
                set((s) => ({
                    tabs: s.tabs.map((t) =>
                        t.id === tabId
                            ? {
                                ...t,
                                sseFrames: [...(t.sseFrames || []), frame].slice(-500),
                            }
                            : t,
                    ),
                }))
            ...
            connectSse(url) {
                ...
                const proto = window.location.protocol === "https:" ? "wss" : "ws"
                const sock = new WebSocket(
                    `${proto}://${window.location.host}/ws?type=${encodeURIComponent(`sse-proxy:${tabId}`)}`,
                )
                sock.onopen = () => {
                    const headerMap = Object.fromEntries(
                        (tab.headers || [])
                ...
                sock.onmessage = (ev) => {
                    ...
                    if (msg.action === "status") {
                        ...
                        if (msg.error) {
                            showNotif(`SSE: ${msg.error}`, "error")
                            appendSseFrame(tabId, {
                                id: uid(),
                                direction: "system",
                                timestamp: Date.now(),
                                data: `Error: ${msg.error}`,
                            ...
                            appendSseFrame(tabId, {
                                ...
                                direction: "system",
                                timestamp: Date.now(),
                                data: msg.connected ? "Connected" : "Stream closed",
                            })
                    } else if (msg.action === "message") {
                        appendSseFrame(tabId, {
                            id: uid(),
                ...
                set((s) => ({
                    wsSockets: { ...s.wsSockets, [`sse:${tabId}`]: sock },
                }))
            },
            disconnectSse() {
                const { getActiveTab, wsSockets } = get()
                ...
                set((s) => ({
                    tabs: s.tabs.map((t) =>
                        t.id === tabId
                            ? {
                                ...t,
                                sioFrames: [...(t.sioFrames || []), frame].slice(-500),
                            }
                            : t,
                    ),
                }))
            ...
            connectSio(url) {
                ...
                const proto = window.location.protocol === "https:" ? "wss" : "ws"
                const sock = new WebSocket(
                    `${proto}://${window.location.host}/ws?type=${encodeURIComponent(`sio-proxy:${tabId}`)}`,
                )
                sock.onopen = () => {
                    const headerMap = Object.fromEntries(
                        (tab.headers || [])
                ...
                sock.onmessage = (ev) => {
                    ...
                    if (msg.action === "status") {
                        ...
                        if (msg.error) {
                            showNotif(`Socket.io: ${msg.error}`, "error")
                            appendSioFrame(tabId, {
                                id: uid(),
                                direction: "system",
                                timestamp: Date.now(),
                                data: `Error: ${msg.error}`,
                        ...
                        appendSioFrame(tabId, {
                            ...
                            direction: msg.direction || "in",
                            timestamp: msg.timestamp || Date.now(),
                            data: `[${msg.event}] ${dataStr}`,
                        })
                    }
                }
                sock.onclose = () => updateTab(tabId, { sioConnected: false })
                set((s) => ({
                    wsSockets: { ...s.wsSockets, [`sio:${tabId}`]: sock },
                }))
            },
            disconnectSio() {
                const { getActiveTab, wsSockets } = get()
            ...
            emitSio(event, data) {
                ...
                if (sock && sock.readyState === WebSocket.OPEN)
                    sock.send(JSON.stringify({ action: "emit", event, data }))
                else showNotif("Not connected", "error")
            },
                ...
                set((s) => ({
                    tabs: s.tabs.map((t) =>
                        t.id === tabId
                            ? {
                                ...t,
                                grpcFrames: [...(t.grpcFrames || []), frame].slice(-500),
                            }
                            : t,
                    ),
                }))
            ...
            grpcSend(action) {
                ...
                if (!sock || sock.readyState >= WebSocket.CLOSING) {
                    const proto = window.location.protocol === "https:" ? "wss" : "ws"
                    sock = new WebSocket(
                        `${proto}://${window.location.host}/ws?type=${encodeURIComponent(`grpc-proxy:${tabId}`)}`,
                    )
                    sock.onmessage = (ev) => {
                        let msg
                        try {
                        ...
                        if (msg.action === "schema") {
                            ...
                            if (msg.error) {
                                showNotif(`gRPC: ${msg.error}`, "error")
                                appendGrpcFrame(tabId, {
                                    id: uid(),
                                    direction: "system",
                                    timestamp: Date.now(),
                                    data: `Error: ${msg.error}`,
            ...
            loadGrpcProto() {
                ...
                const tab = getActiveTab()
                if (!tab.grpcProtoText?.trim()) {
                    showNotif("Paste a .proto file first", "error")
                    return
                }
            ...
            callGrpc() {
                ...
                const tab = getActiveTab()
                if (!tab.url?.trim()) {
                    showNotif("Enter the target address (host:port)", "error")
                    return
                }
                if (!tab.grpcMethodPath?.trim()) {
                    showNotif("Pick a method first", "error")
                    return
                }
                grpcSend({
                    action: "call",
                    address: tab.url.trim(),
                    protoText: tab.grpcProtoText,
                    methodPath: tab.grpcMethodPath,
                    requestJson: tab.grpcRequestJson,
                })
            },
            disconnectGrpc() {
                const { getActiveTab, wsSockets } = get()
            ...
            async saveRequest() {
                ...
                if (!tab.activeReq) {
                    if (!collections.length) {
                        showNotif("Create a collection first", "error")
                        return
                    }
                    const col = collections[0]
                    const req = await apiFetch(`/api/collections/${col.id}/requests`, {
                        method: "POST",
                        body: payload,
                    })
                    ...
                    await apiFetch(
                        `/api/collections/${tab.activeReq.colId}/requests/${tab.activeReq.reqId}`,
            ...
            updatePanelSize(key, size) {
                set((s) => ({
                    panels: { ...s.panels, [key]: { ...s.panels[key], size } },
                }))
            },
            resetPanels() {
            ...
            addIntercepted(entry) {
                        ...
                        filter.target === "url"
                            ...
                            : filter.target === "host"
                                ...
                                : filter.target === "method"
                                    ? entry.method || ""
                                    : filter.target === "content_type"
                                        ? entry.requestHeaders?.["content-type"] || ""
                                        : ""
                    const p = filter.pattern
                    switch (filter.mode) {
                        case "contains":
                            return val.toLowerCase().includes(p.toLowerCase())
                        case "exact":
                            return val === p
                        case "starts_with":
                            return val.toLowerCase().startsWith(p.toLowerCase())
                        case "ends_with":
                            return val.toLowerCase().endsWith(p.toLowerCase())
                        case "regex":
                            try {
                                return new RegExp(p).test(val)
                            } catch {
                                return false
                            }
                        default:
                ...
                const anyMatch = filters.length
                    ? filters.some((f) => matches(entry, f))
                    : false

                if (interceptorFilterMode === "blacklist" && anyMatch) return
                if (
                    interceptorFilterMode === "whitelist" &&
                    filters.length &&
                    !anyMatch
                )
                    return
                ...
                set((s) => ({
                    intercepted: [entry, ...s.intercepted].slice(0, 200),
                }))
            },
            clearIntercepted() {
                set({ intercepted: [] })
            ...
            setFilterMode(mode) {
                set({ interceptorFilterMode: mode })
            },
            addFilter(overrides) {
                set((s) => ({
                    interceptorFilters: [...s.interceptorFilters, makeFilter(overrides)],
                }))
            },
            updateFilter(id, patch) {
                set((s) => ({
                    interceptorFilters: s.interceptorFilters.map((f) =>
                        f.id === id ? { ...f, ...patch } : f,
                    ),
                }))
            },
            deleteFilter(id) {
                set((s) => ({
                    interceptorFilters: s.interceptorFilters.filter((f) => f.id !== id),
                }))
            },
            ...
            showNotif(msg, type = "success") {
                ...
                setTimeout(
                    () => set((s) => (s.notif?.msg === msg ? { notif: null } : s)),
                    2800,
                )
            },
            ...
            async boot() {
                try {
                    const [cols, envs, hist, cfg, pluginTabs, installedPlugins] =
                        await Promise.all([
                            apiFetch("/api/collections"),
                            apiFetch("/api/environments"),
                            apiFetch("/api/history?limit=30"),
                            apiFetch("/api/config"),
                            apiFetch("/api/plugins/ui"),
                    ...
                    set((s) => ({
                        ...
                        pluginUpdates: installedPlugins.filter(
                            (plugin) => plugin.updateAvailable,
                        ),
                        activeEnvId: s.activeEnvId || envs[0]?.id || null,
                        expandedCols: cols.length ? { [cols[0].id]: true } : {},
                    }))
                    const updates = installedPlugins.filter(
                        (plugin) => plugin.updateAvailable,
                    )
                    if (updates.length) {
                        get().showNotif(
                            `${updates.length} plugin update${updates.length === 1 ? "" : "s"} available. Open Plugins to update.`,
                            "info",
                        )
                    }
                } catch { }
            ...
            async loadPluginTabs() {
                try {
                    set({
                        pluginTabs: normalizePluginTabs(await apiFetch("/api/plugins/ui")),
                    })
                } catch { }
            },
        }),

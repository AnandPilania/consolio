...
import {
  Plus, Trash2, Send, Folder, ChevronRight, Settings, History, Zap, Copy, X, Check,
  Eye, EyeOff, Save, Play, Code2, Terminal, Search, Upload, PanelLeft, Filter, Globe,
  GripVertical, ChevronDown, RefreshCw, Ban, Shield, BarChart3, Sparkles, Pencil, Loader2,
} from 'lucide-react'
...
/* ── Icon ─────────────────────────────────────────────────────────────────── */
const ICONS = {
  plus: Plus, trash: Trash2, send: Send, folder: Folder, chevRight: ChevronRight,
  settings: Settings, history: History, zap: Zap, copy: Copy, x: X, check: Check,
  eye: Eye, eyeOff: EyeOff, save: Save, play: Play, code: Code2, terminal: Terminal,
  search: Search, upload: Upload, layout: PanelLeft, filter: Filter, globe: Globe,
  drag: GripVertical, chevDown: ChevronDown, refresh: RefreshCw, ban: Ban, shield: Shield,
  barChart: BarChart3, sparkle: Sparkles, edit: Pencil,
}

export function Icon({ name, size = 14, className, style }) {
  const Cmp = ICONS[name]
  if (!Cmp) return null
  return <Cmp size={size} strokeWidth={1.8} className={className} style={style} />
}
...
/* ── Button ───────────────────────────────────────────────────────────────── */
const VARIANT_MAP = { primary: 'default', ghost: 'outline', danger: 'destructive' }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex-1 overflow-y-auto">
        {rows.map((r, i) => (
          <div key={r.id || i} className="flex items-center gap-1 border-b border-[var(--bd-faint)] px-2 py-0.5 hover:bg-[var(--bg-raised)]">
            <Checkbox checked={r.enabled} onCheckedChange={v => update(i, 'enabled', !!v)} />
            <input
              ...
              placeholder={placeholder[0]} value={r.key || ''} onChange={e => update(i, 'key', e.target.value)}
            />
            <input
              className="flex-1 rounded-sm border border-transparent bg-transparent px-1.5 py-1 font-mono text-[11.5px] text-foreground outline-none transition-colors placeholder:text-[var(--tx-faint)] focus:border-[var(--bd-base)] focus:bg-[var(--bg-overlay)]"
              placeholder={placeholder[1]} value={r.value || ''} onChange={e => update(i, 'value', e.target.value)}
            />
...
export function Input({ value, onChange, placeholder, type = 'text', className }) {
  return <ShInput type={type} className={className} value={value} onChange={onChange} placeholder={placeholder} />
}
...
function JsonNode({ label, isIndex, value, depth }) {
  ...
  const labelSpan = label !== null && (
    <>
      <span className={isIndex ? 'text-[var(--tx-faint)]' : 'tok-key'}>{isIndex ? label : `"${label}"`}</span>
  ...
  return (
    <div>
      <div className="flex cursor-pointer items-center whitespace-pre" style={{ paddingLeft: depth * 14 }} onClick={() => setOpen(o => !o)}>
        <ChevronRight size={9} className={cn('mr-0.5 inline-block shrink-0 text-[var(--tx-faint)] transition-transform', open && 'rotate-90')} />
        {labelSpan}
        <span className="text-[var(--tx-faint)]">{open_}</span>
        {!open && <span className="mx-1 italic text-[var(--tx-faint)]">{entries.length} {isArray ? 'items' : 'keys'}</span>}
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex shrink-0 items-center gap-1.5 border-b border-[var(--bd-faint)] px-2.5 py-1.5">
        <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', connected ? 'bg-[var(--ok)] shadow-[0_0_0_3px_var(--ok-dim)]' : 'bg-[var(--tx-faint)]')} />
        <span className="text-[11px] font-semibold text-muted-foreground">{connected ? 'Connected' : 'Disconnected'}</span>
      ...
      <div className="flex flex-1 flex-col overflow-y-auto">
        {frames.length === 0
          ...
          : frames.map(f => (
            <div key={f.id} className="flex items-start gap-2 border-b border-[var(--bd-faint)] px-2.5 py-1 font-mono text-[11.5px]">
              <span className={cn('w-3 shrink-0', f.direction === 'out' ? 'text-primary' : f.direction === 'in' ? 'text-[var(--ok)]' : 'text-[var(--tx-faint)]')}>
                {f.direction === 'out' ? '→' : f.direction === 'in' ? '←' : '·'}
              </span>
              <span className="shrink-0 pt-px text-[10.5px] text-[var(--tx-faint)]">{new Date(f.timestamp).toLocaleTimeString()}</span>
              <pre className="flex-1 whitespace-pre-wrap break-all text-foreground">{typeof f.data === 'string' ? f.data : JSON.stringify(f.data)}</pre>
            </div>
          ))
        }
      </div>

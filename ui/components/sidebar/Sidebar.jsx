...
export function Sidebar() {
	...
	const activeReq = (tabs.find((t) => t.id === activeTabId) || tabs[0])
		?.activeReq
	...
	const expand = (id) =>
		useStore.setState((s) => ({
			expandedCols: { ...s.expandedCols, [id]: true },
		}))
	...
	const addRequest = async (colId, e, folderId = null) => {
		...
		const req = await apiFetch(`/api/collections/${colId}/requests`, {
			method: "POST",
			body: { name: "New Request", method: "GET", url: "", folderId },
		})
	...
	const submitNewFolder = async (colId, parentId, name) => {
		if (name.trim()) {
			await apiFetch(`/api/collections/${colId}/folders`, {
				method: "POST",
				body: { name: name.trim(), parentId },
			})
			await refreshCollections()
		}
	...
	const deleteFolder = async (colId, folderId, e) => {
		e.stopPropagation()
		await apiFetch(`/api/collections/${colId}/folders/${folderId}`, {
			method: "DELETE",
		})
		await refreshCollections()
	...
	const moveRequest = async (colId, reqId, folderId) => {
		await apiFetch(`/api/collections/${colId}/requests/${reqId}`, {
			method: "PUT",
			body: { folderId },
		})
		await refreshCollections()
	...
	const exportCollection = (col, format) => {
		...
		downloadJson(
			`${(col.name || "collection").replace(/\s+/g, "_")}.${format}.json`,
			data,
		)
	}
		...
		collections.forEach((col) => {
			...
			apiFetch(`/api/collections/${col.id}/score`)
				.then((s) =>
					setScores((prev) => ({
						...prev,
						[col.id]: { score: s.score, grade: s.grade },
					})),
				)
				.catch(() => {})
		})
	...
	const reqItemClass =
		"mx-1.5 flex items-center gap-1.5 rounded-sm py-1 pr-1.5 cursor-pointer transition-colors hover:bg-[var(--bg-raised)]"
	...
	const renderNewFolderInput = (colId, parentId, depth) => (
		...
		>
			...
			<input
				...
				onKeyDown={(e) => {
					if (e.key === "Enter")
						submitNewFolder(colId, parentId, e.target.value)
	...
	const renderRequestRow = (col, req, depth) => {
		...
		return (
			<div
				...
				onKeyDown={(e) => e.key === "Enter" && loadRequest(col, req)}
			>
				<MethodBadge method={req.method || "GET"} small />
				<span
					className={cn(
						"flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-[11.5px] text-muted-foreground",
				...
				{col.folders?.length > 0 && (
					<select
						...
						onClick={(e) => e.stopPropagation()}
						onChange={(e) =>
							moveRequest(col.id, req.id, e.target.value || null)
						}
	...
	const renderFolderNode = (col, folder, depth) => {
		...
		const childFolders = (col.folders || []).filter(
			(f) => f.parentId === folder.id,
		)
		const childRequests = (col.requests || []).filter(
			(r) => (r.folderId || null) === folder.id,
		)
		return (
			<div key={folder.id}>
				<div
					...
					onKeyDown={(e) => e.key === "Enter" && toggleCol(folder.id)}
				>
					<ChevronRight
						size={10}
						className={cn(
							"shrink-0 text-[var(--tx-faint)] transition-transform",
							isOpen && "rotate-90",
						)}
					/>
					<Folder size={11} className="shrink-0 text-[var(--tx-faint)]" />
					<span className="flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-[11.5px] font-semibold text-muted-foreground">
						{folder.name}
					</span>
					<IconBtn
						name="plus"
						size={10}
						title="Add request"
						onClick={(e) => addRequest(col.id, e, folder.id)}
					/>
					<IconBtn
						name="folder"
						size={10}
						title="Add subfolder"
						onClick={(e) => startCreateFolder(col.id, folder.id, e)}
					/>
					<IconBtn
						name="trash"
						size={10}
						title="Delete folder"
						onClick={(e) => deleteFolder(col.id, folder.id, e)}
						danger
					/>
				</div>
				{isOpen && (
					<div className="py-px pb-1">
						...
						{creatingFolderIn?.colId === col.id &&
							creatingFolderIn?.parentId === folder.id &&
							renderNewFolderInput(col.id, folder.id, depth + 1)}
	...
	const loadIntercepted = (item) => {
		useStore.getState().updateActiveTab({
			...
			url: item.url || "",
			headers: Object.entries(item.requestHeaders || {}).map(([k, v]) => ({
				id: uid(),
				key: k,
				value: v,
				enabled: true,
		...
		<aside className="flex h-full flex-col overflow-hidden bg-[var(--bg-surface)]">
			...
			<div className="flex shrink-0 items-center gap-0.5 border-b border-[var(--bd-faint)] px-2 py-1.5">
				...
				{sbTab === "collections" && (
					<>
						<IconBtn
							name="upload"
							size={13}
							title="Import cURL / Postman"
							onClick={() => useStore.setState({ modal: "import" })}
						/>
						<IconBtn
							name="play"
							size={13}
							title="Run collection"
							onClick={() => useStore.setState({ modal: "runner" })}
						/>
						<IconBtn
							name="plus"
							size={13}
				...
				{sbTab === "interceptor" && (
					<IconBtn
						name="x"
						size={13}
						title="Clear all intercepted"
						onClick={() => useStore.getState().clearIntercepted()}
					/>
				)}
			</div>
			...
			<div className="flex-1 overflow-y-auto py-1.5">
					...
					<>
						{collections.length === 0 && (
							<div className="px-4 py-5 text-center text-[12px] leading-[1.7] text-[var(--tx-faint)]">
								...
								<button
									type="button"
									className="mt-1.5 inline-block text-[12px] text-primary underline"
									onClick={() =>
										useStore.setState({
											modal: "newCollection",
											modalData: {},
						...
						{collections.map((col) => (
							<div key={col.id} className="mb-px px-1.5">
								<div
									role="option"
									aria-selected={!!expandedCols[col.id]}
									className="flex cursor-pointer items-center gap-1.5 rounded-sm px-1.5 py-1 transition-colors hover:bg-[var(--bg-raised)]"
									onClick={() => toggleCol(col.id)}
									onKeyDown={(e) => e.key === "Enter" && toggleCol(col.id)}
									tabIndex={0}
								>
									<ChevronRight
										size={11}
										className={cn(
											"shrink-0 text-[var(--tx-faint)] transition-transform",
											expandedCols[col.id] && "rotate-90",
										)}
									/>
									<Folder size={12} className="shrink-0 text-primary" />
									<span className="flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-[11.5px] font-semibold text-foreground">
										{col.name}
									</span>
									<span className="rounded-full bg-[var(--bg-overlay)] px-1.5 py-px text-[10px] text-[var(--tx-faint)]">
									...
									<select
										...
										onClick={(e) => e.stopPropagation()}
										onChange={(e) => {
											if (e.target.value) exportCollection(col, e.target.value)
											e.target.value = ""
										}}
									...
									<IconBtn
										...
										size={11}
										title="Add folder"
										onClick={(e) => startCreateFolder(col.id, null, e)}
									/>
									<IconBtn
										name="trash"
										size={11}
								...
								{expandedCols[col.id] && (
									<div className="py-px pb-1">
										{(col.folders || [])
											.filter((f) => !f.parentId)
											.map((f) => renderFolderNode(col, f, 0))}
										{creatingFolderIn?.colId === col.id &&
											creatingFolderIn?.parentId === null &&
											renderNewFolderInput(col.id, null, 0)}
										{(col.requests || [])
											.filter((r) => !r.folderId)
											.map((req) => renderRequestRow(col, req, 0))}
										{col.requests?.length === 0 && !col.folders?.length && (
											<p className="px-6 py-1 text-[10.5px] text-[var(--tx-faint)]">
												No requests yet
											</p>
										)}
									</div>
				...
				{sbTab === "history" && (
					<>
						{history.length === 0 && (
							<p className="px-4 py-5 text-center text-[12px] leading-[1.7] text-[var(--tx-faint)]">
								No history yet
							</p>
						)}
						{history.map((h) => (
							<div
								...
								aria-selected={false}
								tabIndex={0}
								className="flex items-center gap-1.5 border-b border-[var(--bd-faint)] px-2.5 py-1 cursor-pointer transition-colors hover:bg-[var(--bg-raised)]"
								onClick={() => loadHistory(h)}
								onKeyDown={(e) => e.key === "Enter" && loadHistory(h)}
							>
								<MethodBadge method={h.request.method || "GET"} small />
								<StatusChip status={h.response?.status} />
								<span className="flex-1 overflow-hidden text-ellipsis whitespace-nowrap font-mono text-[10.5px] text-[var(--tx-faint)]">
									{h.request.url || ""}
								</span>
								<span className="shrink-0 whitespace-nowrap text-[10px] text-[var(--tx-faint)]">
									{timeAgo(h.timestamp)}
								</span>
							</div>
						))}
...
function GradeChip({ grade, title, onClick }) {
	const cls =
		grade === "A" || grade === "B"
			? "text-[var(--ok)] bg-[var(--ok-dim)]"
			: grade === "C" || grade === "D"
				? "text-[var(--warn)] bg-[rgba(251,191,36,0.12)]"
				: "text-[var(--err)] bg-[var(--err-dim)]"
	return (
		<button
			type="button"
			className={cn(
				"cursor-pointer rounded-full px-1.5 py-px text-[9.5px] leading-[1.4] font-bold",
				cls,
			)}
			title={title}
			onClick={onClick}
...
function StatusChip({ status }) {
	const cls = !status
		...
		: status < 300
			? "text-[var(--ok)] bg-[var(--ok-dim)]"
			: status < 400
				? "text-[var(--info)] bg-[color-mix(in_srgb,var(--info)_12%,transparent)]"
				: status < 500
					? "text-[var(--warn)] bg-[color-mix(in_srgb,var(--warn)_12%,transparent)]"
					: "text-[var(--err)] bg-[var(--err-dim)]"
	return (
		<span
			className={cn(
				"shrink-0 rounded-[3px] px-1.5 py-px font-mono text-[10px] font-semibold",
				cls,
			)}
		>
			{status || "—"}
	...
	return (
		<div className="flex flex-col">
			...
			<div className="flex flex-col gap-1 px-2 py-1.5">
				{filters.map((f) => (
					<div key={f.id} className="flex items-center gap-0.5">
						...
						<button
							type="button"
							className="shrink-0 flex items-center text-[var(--tx-faint)] transition-colors hover:text-destructive"
				...
				<button
					type="button"
					className="mt-1 flex items-center justify-center gap-1 rounded-sm border border-dashed border-[var(--bd-subtle)] py-1 text-[11px] text-[var(--tx-faint)] transition-colors hover:border-primary hover:text-primary"
					onClick={() => addFilter()}
				>
			...
			{intercepted.length === 0 ? (
				<p className="mt-3 px-4 py-5 text-center text-[12px] leading-[1.7] text-[var(--tx-faint)]">
					Waiting for requests…
				</p>
			) : (
				intercepted.map((item) => (
					<div
						key={`${item.method || "GET"}-${item.url || "request"}-${item.timestamp || "captured"}`}
						role="option"
						tabIndex={0}
						className="cursor-pointer border-b border-[var(--bd-faint)] px-2.5 py-1.5 transition-colors hover:bg-[var(--bg-raised)]"
						onClick={() => onLoad(item)}
					...
					>
						...
						<div className="overflow-hidden text-ellipsis whitespace-nowrap font-mono text-[10.5px] text-[var(--tx-faint)]">
							{item.url}
						</div>
					</div>
				))

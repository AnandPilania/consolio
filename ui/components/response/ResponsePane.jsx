...
import { cx, diffLines, fmtSize, fmtTime, syntaxHighlight } from "../../utils"
import { Empty, IconBtn, JsonTree, MessageLogView, Spinner } from "../shared"
...
export function ResponsePane() {
	...
	const previewLiveRender = useStore(
		(s) => s.config?.settings?.previewLiveRender === true,
	)
	...
	const prevEntry = useMemo(() => {
		...
		return (
			history.find(
				(h) =>
					h.id !== response.historyId &&
					h.request?.method === tab.method &&
					h.request?.url === response.finalUrl,
			) || null
		)
	...
	const previewableType = response
		? ["html", "image"].includes(response.bodyType)
		: false
	const imageMime =
		response?.headers?.["content-type"]?.split(";")[0]?.trim() || "image/png"
	const responsePluginTab = pluginTabs.find(
		(pluginTab) =>
			`plugin:${pluginTab.plugin}:${pluginTab.id}` === pluginTabKey,
	)
	const selectNativeTab = (key) => {
		setPluginTabKey(null)
		setResTab(key)
	}
	...
	if (isWs)
		return (
			<div className="flex h-full flex-col overflow-hidden bg-background">
				...
				<MessageLogView
					frames={tab.wsFrames}
					connected={tab.wsConnected}
					onClear={clearWsFrames}
					emptyText="Not connected — press Connect to open the WebSocket"
				/>
				{tab.wsConnected && (
					<ComposeBar>
						<input
	...
	if (isSse)
		return (
			<div className="flex h-full flex-col overflow-hidden bg-background">
				...
				<MessageLogView
					frames={tab.sseFrames}
					connected={tab.sseConnected}
					onClear={clearSseFrames}
					emptyText="Not connected — press Connect to open the event stream"
				/>
			</div>
		)
	...
	if (isSio)
		return (
			<div className="flex h-full flex-col overflow-hidden bg-background">
				...
				<MessageLogView
					frames={tab.sioFrames}
					connected={tab.sioConnected}
					onClear={clearSioFrames}
					emptyText="Not connected — press Connect to open the Socket.IO connection"
				/>
				{tab.sioConnected && (
					<ComposeBar>
						<input
	...
	if (isGrpc)
		return (
			<div className="flex h-full flex-col overflow-hidden bg-background">
				<Header label="gRPC" />
				<MessageLogView
					frames={tab.grpcFrames}
					connected={tab.grpcConnected}
					onClear={clearGrpcFrames}
					emptyText="Load a .proto, pick a method, then press Call"
				/>
			</div>
		)
	...
	if (!response)
		return (
			<div className="flex h-full flex-col overflow-hidden bg-background">
				...
				<Empty
					icon="⚡"
					text="Hit Send to make a request"
					sub="Response will appear here"
				/>
			</div>
		)
	...
	return (
		<div className="flex h-full flex-col overflow-hidden bg-background">
			...
			<div className="flex h-9 shrink-0 items-center gap-2 border-b border-[var(--bd-faint)] bg-[var(--bg-surface)] px-3">
				<span
					className={cn(
						"rounded-sm px-2 py-0.5 font-mono text-[12px] font-bold",
						statusCls,
					)}
				>
				...
				<div className="flex items-center gap-2.5 text-[11px] text-[var(--tx-faint)]">
					<span className="flex items-center gap-0.5">
						<Zap size={10} /> {fmtTime(response.elapsed)}
					</span>
					<span>{fmtSize(response.size || 0)}</span>
					{response.redirected && (
						<span className="text-[var(--info)]">↪ redirected</span>
					)}
				</div>
			...
			<div className="flex shrink-0 border-b border-[var(--bd-faint)] bg-[var(--bg-surface)] px-3">
				{previewableType && (
					<TabBtn
						active={!responsePluginTab && resTab === "preview"}
						onClick={() => selectNativeTab("preview")}
					>
						<Eye size={12} /> Preview
					</TabBtn>
				)}
				<TabBtn
					active={!responsePluginTab && resTab === "body"}
					onClick={() => selectNativeTab("body")}
				>
					Body
				</TabBtn>
				<TabBtn
					active={!responsePluginTab && resTab === "headers"}
					onClick={() => selectNativeTab("headers")}
				>
					Headers{" "}
					<TabBadge>{Object.keys(response.headers || {}).length}</TabBadge>
				</TabBtn>
				{totalTests > 0 && (
					<TabBtn
						active={!responsePluginTab && resTab === "tests"}
						onClick={() => selectNativeTab("tests")}
					>
						Tests{" "}
						<TabBadge
							className={
				...
				{prevEntry && (
					<TabBtn
						active={!responsePluginTab && resTab === "diff"}
						onClick={() => selectNativeTab("diff")}
					>
						Diff
					</TabBtn>
				)}
			...
			<div className="flex-1 overflow-auto">
				...
				{!responsePluginTab &&
					...
					Object.entries(response.headers || {}).map(([k, v]) => (
						<div
							key={k}
							className="flex border-b border-[var(--bd-faint)] px-3.5 py-1 font-mono text-[12px] hover:bg-[var(--bg-raised)]"
						>
							<span className="w-[220px] shrink-0 overflow-hidden text-ellipsis text-[var(--tx-faint)]">
								{k}
							</span>
				...
				{!responsePluginTab &&
					...
					testResults.map((r, _i) => {
						...
						return (
							<div
								key={`${r.type}-${r.path || ""}-${r.value || ""}`}
								className={cn(
									"flex items-center gap-2 border-b border-[var(--bd-faint)] px-3.5 py-1.5 text-[12px] hover:bg-[var(--bg-raised)]",
									rowCls,
								)}
							>
								<span
								...
								<span className="font-mono text-[10.5px] text-[var(--tx-faint)]">
									{r.actual}
								</span>
							</div>
						)
				...
				{!responsePluginTab && resTab === "diff" && prevEntry && (
					<DiffView
						oldBody={prevEntry.response?.body || ""}
				...
				{responsePluginTab && (
					<PluginTabContent
						pane="response"
						tab={responsePluginTab}
						context={{ request: tab, response }}
					/>
				)}
			</div>
		</div>
...
function Header({ label }) {
	return (
		<div className="flex h-9 shrink-0 items-center gap-2 border-b border-[var(--bd-faint)] bg-[var(--bg-surface)] px-3">
			<span className="text-[10.5px] font-bold tracking-wide text-[var(--tx-faint)] uppercase">
				{label}
			</span>
		</div>
	)
...
function ComposeBar({ children }) {
	return (
		<div className="flex shrink-0 items-center gap-1.5 border-t border-[var(--bd-faint)] px-2.5 py-2">
			{children}
		</div>
	)
}
...
function DiffView({ oldBody, newBody }) {
	...
	if (lines.every((l) => l.type === "same"))
		return <Empty icon="✓" text="Identical to the previous response" />
	return (
		<div className="py-2 font-mono text-[12px] leading-[1.6]">
			{lines.map((l, _i) => (
				<div
					key={`${l.type}-${l.line}`}
					className={cx(
						"flex gap-2 px-3.5 whitespace-pre-wrap break-all",
						l.type === "add" && "bg-[var(--ok-dim)]",
						l.type === "del" && "bg-[var(--err-dim)]",
					)}
				>
					<span
						className={cn(
							"w-2.5 shrink-0 text-[var(--tx-faint)]",
							l.type === "add" && "text-[var(--ok)]",
							l.type === "del" && "text-[var(--err)]",
						)}
					>
	...
	if (bodyType === "image") {
		return (
			<div
				...
				style={{
					background:
						"repeating-conic-gradient(var(--bg-raised) 0% 25%, var(--bg-overlay) 0% 50%) 50% / 16px 16px",
				}}
			>
				<img
...
function LiveFrame({ url }) {
	...
	return (
		<div className="relative h-full">
			<iframe
				key={
					url /* force remount on URL change so the load/blocked state resets cleanly */
				}
			...
			/>
			{slow && !blocked && !loaded && (
				<div className="absolute top-2 right-2 rounded-md border border-[var(--bd-subtle)] bg-[var(--bg-overlay)] px-2.5 py-1 text-[11px] text-[var(--tx-faint)]">
					Still loading…
				</div>
			)}
			{blocked && !loaded && (
				<div className="absolute inset-0 flex flex-col items-center justify-center bg-background p-6">
					...
					<p className="shrink-0 px-6 pt-2 pb-3.5 text-center text-[11.5px] text-[var(--tx-faint)]">
						...
						in Settings to view the captured response instead — that sandboxed
						view isn't affected by this.
					</p>
				</div>
			)}
		</div>
...
function BodyView({ body, bodyType, search }) {
	...
	useEffect(() => {
		setMode(parsedJson !== undefined ? "tree" : "raw")
	}, [parsedJson])

	const highlighted = useMemo(() => {
		if (!body) return ""
		if (parsedJson !== undefined)
			return syntaxHighlight(JSON.stringify(parsedJson, null, 2))
		return body
			.replace(/&/g, "&amp;")
			.replace(/</g, "&lt;")
			.replace(/>/g, "&gt;")
	...
	if (bodyType === "image") {
		return (
			<Empty
				...
				sub="Switch to the Preview tab to view it"
			/>
		)
	}
	...
	return (
		<div className="h-full">
			{parsedJson !== undefined && !search && (
				<div className="flex gap-0.5 px-3.5 pt-2">
					<button
						type="button"
						className={cn(
							"rounded-full border border-[var(--bd-subtle)] px-2.5 py-0.5 text-[10.5px] font-semibold text-[var(--tx-faint)] transition-colors hover:text-muted-foreground",
							mode === "tree" &&
			...
			{showTree ? (
				...
				<pre className="px-3.5 py-3 font-mono text-[12px] leading-[1.65] whitespace-pre-wrap break-all text-foreground">
					{withSearch.replace(
						/<span class="tok-(?:key|str|num|bool|null)">|<\/span>/g,
						"",
					)}
				</pre>
			)}

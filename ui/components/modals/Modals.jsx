				...
				return (
					<button
						...
						className={cn(
							...
							key === activeKey &&
								"border-transparent bg-[var(--accent-dim)] text-primary!",
						)}
		...
		>
			...
			<FormGroup label="Description (optional)">
				<Input
					...
					placeholder="What APIs does this test?"
				/>
			</FormGroup>
		</Modal>
	)
...
async function createCollectionFromImport(imported, description) {
	...
	if (requests.length) {
		await apiFetch(`/api/collections/${col.id}/requests/bulk`, {
			method: "POST",
			body: { requests },
		})
	}
...
export function ImportModal() {
	...
	const _doScan = async () => {
		...
		setScanning(true)
		try {
			const _result = await apiFetch("/api/scan/routes", {
				method: "POST",
				body: { path: scanPath || undefined, baseUrl: scanBaseUrl },
			})
			if (_result.error) throw new Error(_result.error)
			if (!_result.requests.length)
				throw new Error(
					"No routes found — check the path, or this framework style may not be supported yet (Express, Fastify, and NestJS route decorators are supported).",
				)
	...
	const confirmScanImport = async () => {
		...
		const count = await createCollectionFromImport(
			scanResult,
	...
	const doImport = async () => {
		...
		try {
			if (tab === "curl") {
				...
				if (!req.url)
					throw new Error("Could not parse a URL from this cURL command")
				...
				if (!json.info || !json.item)
					throw new Error(
						"Not a valid Postman collection (needs info + item fields)",
					)
				const count = await createCollectionFromImport(
				...
				if (!json.resources)
					throw new Error(
						'Not a valid Insomnia export (needs a "resources" field)',
					)
				const count = await createCollectionFromImport(
					importInsomniaExport(json),
	...
	const hints = {
		...
		postman:
			"Paste the full contents of an exported Postman collection JSON file. Folders are preserved.",
		insomnia:
			"Paste the full contents of an Insomnia v4 export (Export → resources). Folders are preserved.",
		openapi:
			"Paste an OpenAPI/Swagger 3.x document (JSON or YAML). Requests are grouped into folders by tag.",
	...
	const footer =
		tab === "scan" ? (
			<>
				...
				{!scanResult ? (
					<Btn variant="primary" onClick={_doScan} disabled={scanning}>
						{scanning ? <Spinner size={12} /> : "Scan"}
					</Btn>
				) : (
					<Btn variant="primary" onClick={confirmScanImport}>
						Import {scanResult.requests.length} route
						{scanResult.requests.length === 1 ? "" : "s"}
					</Btn>
				)}
			</>
		) : (
	...
	return (
		<Modal title="Import" icon="upload" onClose={close} footer={footer}>
				...
				<div className="flex flex-col gap-2.5">
					...
					{scanResult && (
						<div className="mt-1.5 max-h-64 overflow-y-auto rounded-md border border-[var(--bd-subtle)]">
							<p className="m-0 border-b border-[var(--bd-faint)] px-3 py-2 text-[11px] text-[var(--tx-faint)]">
								{scanResult.description}
							</p>
							{scanResult.requests.map((r) => (
								<div
									key={r.id}
									className="flex items-center gap-2 border-b border-[var(--bd-faint)] px-3 py-1.5 last:border-b-0"
								>
									<MethodBadge method={r.method} small />
									<span className="overflow-hidden text-ellipsis whitespace-nowrap font-mono text-[11.5px] text-foreground">
										{r.url}
									</span>
								</div>
							))}
			...
			{error && (
				<div className="rounded-sm border border-destructive/25 bg-[var(--err-dim)] px-3 py-2 text-[12px] text-destructive">
					{error}
				</div>
			)}
		</Modal>
...
export function _CodeGenModal() {
	...
	return (
		...
		>
			<PillTabs
				items={GENERATE_TARGETS}
				activeKey={targetIdx}
				onChange={setTargetIdx}
				getKey={(t, _i) => GENERATE_TARGETS.indexOf(t)}
				getLabel={(t) => t.label}
			/>
			{error ? (
				<div className="rounded-sm border border-destructive/25 bg-[var(--err-dim)] px-3 py-2 text-[12px] text-destructive">
				...
				<Textarea
					className="min-h-[280px] resize-y text-[11.5px] leading-[1.55]"
					readOnly
					value={code}
				/>
			)}
...
export function _RunnerModal() {
	...
	const run = async () => {
		...
		const runOne = async (req, i) => {
			if (bailed) {
				setResults((prev) =>
					prev.map((r, j) => (j === i ? { ...r, _status: "skipped" } : r)),
				)
				return
			}
			setResults((prev) =>
				prev.map((r, j) => (j === i ? { ...r, _status: "running" } : r)),
			)
			try {
				...
				const pass =
					!res.error &&
					(hasTests ? res.testResults.every((t) => t.pass) : res.status < 400)
					...
					prev.map((r, j) =>
						j === i
							? {
									...
									_resStatus: res.status,
									_elapsed: res.elapsed,
									_testResults: res.testResults || [],
									_error: res.error,
								}
							: r,
					),
		...
		for (let i = 0; i < reqs.length; i += step) {
			await Promise.all(
				reqs.slice(i, i + step).map((req, k) => runOne(req, i + k)),
			)
			if (bailed) break
			if (delay > 0 && i + step < reqs.length)
	...
	const exportResults = (format) => {
		const items = results.map((r) => ({
			name: r.name || r.url || "Unnamed",
			pass: r._status === "pass",
			elapsed: r._elapsed || 0,
			error:
				r._error ||
				(r._testResults || [])
					.filter((t) => !t.pass)
					.map((t) => `${t.type}: expected ${t.value}, got ${t.actual}`)
					.join("; "),
		}))
		if (format === "json") downloadJson(`${col.name}.results.json`, items)
		else
			downloadText(
				`${col.name}.results.junit.xml`,
				buildJUnitXml(col.name, items),
				"application/xml",
			)
	}
		...
		>
			<div className="flex items-center gap-2">
				...
				<div className="flex shrink-0 items-center gap-1.5 text-[11px] text-[var(--tx-faint)]">
					...
					<input
						type="number"
						className="w-[60px] rounded-sm border border-[var(--bd-subtle)] bg-[var(--bg-raised)] px-1.5 py-1 text-right text-[11.5px] text-foreground focus:border-primary focus:outline-none"
						min={1}
						max={20}
						value={concurrency}
				...
				<label className="flex shrink-0 cursor-pointer items-center gap-1.5 text-[11px] text-[var(--tx-faint)]">
					<input
						type="checkbox"
						checked={bail}
						onChange={(e) => setBail(e.target.checked)}
					/>
				...
				<div className="flex items-center gap-3.5 rounded-md border border-[var(--bd-faint)] bg-[var(--bg-raised)] px-3 py-2 text-[12px]">
					...
					{!running && (
						<>
							...
							<button
								type="button"
								className="text-[var(--tx-faint)] transition-colors hover:text-foreground"
			...
			<div className="flex max-h-80 flex-col gap-1 overflow-y-auto">
				{results.length === 0 ? (
					<p className="p-6 text-center text-[12px] text-[var(--tx-faint)]">
						Select a collection and press Run All
					</p>
				) : (
					results.map((r) => (
						<div
							key={`${r.requestId || r.name || r.url || "result"}-${r._status || "pending"}`}
							className="flex items-center gap-2 rounded-sm border border-[var(--bd-faint)] bg-[var(--bg-raised)] px-2.5 py-1.5"
						>
							<div
								className={cn(
							...
							<span className="flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-[12px] text-foreground">
								{r.name || r.url || `Request ${i + 1}`}
							</span>
							{r._testResults?.length > 0 && (
								<span className="shrink-0 font-mono text-[11px] text-[var(--tx-faint)]">
									{r._testResults.filter((t) => t.pass).length}/
									{r._testResults.length} tests
								</span>
							)}
							{r._resStatus !== undefined && r._resStatus !== null && (
								<span className="shrink-0 font-mono text-[11px] text-[var(--tx-faint)]">
									{r._resStatus} · {fmtTime(r._elapsed || 0)}
								</span>
							)}
						</div>
					))
...
/* ── Settings ─────────────────────────────────────────────────────────────── */
export function _SettingsModal() {
	...
	const saveEnv = async () => {
		...
		const payload = {
			name: envForm.name,
			color: envForm.color || "#6366f1",
			variables: envForm.variables || [],
		}
		if (envEdit === "__new__") {
			const created = await apiFetch("/api/environments", {
				method: "POST",
				body: payload,
			})
			useStore.setState({ activeEnvId: created.id })
		} else {
			await apiFetch(`/api/environments/${envEdit}`, {
				method: "PUT",
				body: payload,
			})
		}
	...
	return (
		...
		>
			...
			<section className="flex flex-col gap-3">
				<h3 className="m-0 text-[11px] font-bold tracking-wide text-[var(--tx-faint)] uppercase">
					Project
				</h3>
						...
						<Select
							...
							onChange={(e) =>
								setForm((f) => ({
									...
									settings: {
										...(f.settings || {}),
										previewLiveRender: e.target.value === "true",
									},
								}))
							}
						>
							<option value="false">
								Off — sandboxed, isolated from consolio
							</option>
							<option value="true">
								On — full app rendering (SPAs, cookies)
							</option>
						</Select>
					</FormGroup>
					<p className="col-span-full -mt-1 text-[11px] leading-[1.5] text-[var(--tx-faint)]">
						When on, GET responses previewed as HTML navigate the browser
						directly to the request URL — needed for apps with their own
						scripts, cookies, and cross-origin assets to render correctly. That
						preview frame can then read and write cookies/storage for that site,
						same as a normal browser tab. Off is safer for previewing untrusted
						or third-party APIs.
					</p>
				</div>
			</section>
			...
			<section className="flex flex-col gap-3">
				...
				<div className="grid grid-cols-2 gap-3">
					...
					<FormGroup
						label={
							aiConfig.provider === "anthropic" ? "Model (optional)" : "Model"
						}
					>
						<Input
				...
				<p className="-mt-1 text-[11px] leading-[1.5] text-[var(--tx-faint)]">
					Powers the "Fix with AI" button on a request's Info tab, which
					suggests a description and test assertions. Bring-your-own-endpoint —
					works with Anthropic, OpenAI, Azure OpenAI, a local or remote Ollama
					instance, or any other OpenAI-Chat-Completions-compatible server.
					Settings are stored only in this browser's local storage, sent only
					when you click that button, and never saved to the project or to
					consolio's own storage.
				</p>
			</section>
			...
			<section className="flex flex-col gap-3">
				<div className="flex items-center">
					<h3 className="m-0 flex-1 text-[11px] font-bold tracking-wide text-[var(--tx-faint)] uppercase">
						Environments
					</h3>
					<Btn
						variant="ghost"
				...
				<div className="flex flex-col gap-1.5">
					{environments.map((env) => (
						<div
							key={env.id}
							className="flex items-center gap-1.5 rounded-sm border border-[var(--bd-faint)] bg-[var(--bg-raised)] px-2.5 py-1.5"
						>
							<span
							...
							{activeEnvId === env.id && (
								<span className="rounded-full bg-[var(--ok-dim)] px-1.5 py-px text-[10px] text-[var(--ok)]">
									active
								</span>
							)}
							<button
								type="button"
								className="flex items-center p-0.5 text-[var(--tx-faint)] transition-colors hover:text-foreground"
								title="Set active"
								onClick={() => useStore.setState({ activeEnvId: env.id })}
							>
								✓
							</button>
							<button
							...
							<button
								type="button"
								className="flex items-center p-0.5 text-[var(--tx-faint)] transition-colors hover:text-destructive!"
								title="Delete"
					...
					{environments.length === 0 && (
						<p className="text-[12px] text-[var(--tx-faint)]">
							No environments yet
						</p>
					)}
				</div>
				...
				{envEdit && (
					<div className="flex flex-col gap-2.5 rounded-md border border-[var(--bd-base)] bg-[var(--bg-raised)] p-3.5">
						...
						<div className="text-[10.5px] font-semibold tracking-wide text-[var(--tx-faint)] uppercase">
							Variables
						</div>
						...
						<div className="flex items-center gap-2">
							...
							<Btn variant="ghost" size="sm" onClick={() => setEnvEdit(null)}>
								Cancel
							</Btn>
							<Btn variant="primary" size="sm" onClick={saveEnv}>
								Save
							</Btn>
						</div>
					</div>
			...
			<section className="flex flex-col gap-3">
				...
				<div className="flex flex-col gap-1.5 rounded-md border border-[var(--bd-faint)] bg-[var(--bg-raised)] px-3.5 py-3 text-[12px] leading-[1.7] text-muted-foreground [&_code]:rounded-[3px] [&_code]:bg-[var(--bg-overlay)] [&_code]:px-1.5 [&_code]:py-px [&_code]:text-primary">
					<p className="m-0">
						1. Open <code>chrome://extensions/</code> → enable{" "}
						<strong>Developer Mode</strong>
					</p>
					<p className="m-0">
						2. Click <strong>Load unpacked</strong> → select the{" "}
					...
					<p className="m-0">
						4. Toggle <strong>Capture requests</strong> ON — requests appear in
						the <strong>Tap</strong> sidebar
					</p>
				</div>
			</section>
		</Modal>
...
/* ── Plugin manager ───────────────────────────────────────────────────────── */
export function _PluginManagerModal() {
	const showNotif = useStore((s) => s.showNotif)
	...
	useEffect(() => {
		...
		pluginNodes.current[name].scrollIntoView({
			block: "center",
			behavior: "smooth",
		})
	...
	}, [modalData?.pluginName])

	const install = async () => {
		if (!pkgName.trim()) {
			showNotif("Enter a package name", "error")
			return
		}
		setInstalling(true)
		try {
	...
	const installBundled = async (dir) => {
		...
		try {
			const res = await apiFetch("/api/plugins/bundled", {
				method: "POST",
				body: { dir },
			})
			if (res.error) throw new Error(res.error)
	...
	const toggleEnabled = async (p) => {
		await apiFetch(`/api/plugins/${p.name}`, {
			method: "PUT",
	...
	const updatePlugin = async (plugin) => {
		...
		setInstallingDirs((current) => new Set(current).add(key))
		try {
			const res = await apiFetch("/api/plugins/update", {
				method: "POST",
				body: { name: plugin.name },
			})
			if (res.error) throw new Error(res.error)
			showNotif(`Updated ${plugin.name}`, "success")
			await loadPlugins()
		} catch (e) {
			showNotif(e.message, "error")
		} finally {
		...
		<>
			...
			>
				...
				{outdatedPlugins.length > 0 && (
					<div className="rounded-sm border border-[var(--warn)] bg-[var(--warn-dim)] px-2.5 py-2 text-[11.5px] text-foreground">
						{outdatedPlugins.length} plugin update
						{outdatedPlugins.length === 1 ? "" : "s"} available. Update them
						below to load the latest shipped code.
					</div>
				)}

				{!loading && notYetInstalled.length > 0 && (
					<>
						<p className="mt-3.5 text-[11px] leading-[1.5] text-[var(--tx-faint)]">
							Bundled with consolio — install with one click:
						</p>
						<div className="flex flex-col gap-1">
							{notYetInstalled.map((b) => (
								<div
									key={b.dir}
									ref={setPluginNode(b.name)}
									className={cn(
										"flex cursor-default items-center gap-2 rounded-sm border border-[var(--bd-faint)] bg-[var(--bg-raised)] px-2 py-1.5 transition-colors",
										focusedPlugin === b.name &&
											"border-primary bg-[var(--accent-dim)]",
									)}
								>
									...
									<Btn
										variant="ghost"
										size="sm"
										onClick={() => installBundled(b.dir)}
										disabled={installingDirs.has(b.dir)}
									>
										{installingDirs.has(b.dir) ? (
											<Spinner size={12} />
										) : (
											"Install"
										)}
				...
				<div className="mt-3 flex flex-col gap-1">
					...
					{plugins.map((p) => (
						<div
							key={p.name}
							ref={setPluginNode(p.name)}
							className={cn(
								"flex cursor-default items-center gap-2 rounded-sm border border-[var(--bd-faint)] bg-[var(--bg-raised)] px-2 py-1.5 transition-colors",
								focusedPlugin === p.name &&
									"border-primary bg-[var(--accent-dim)]",
							)}
						>
							<span
							...
							{p.updateAvailable && bundledNames.has(p.name) && (
								<Btn
									variant="ghost"
									size="sm"
									onClick={() => updatePlugin(p)}
									disabled={installingDirs.has(`update:${p.name}`)}
								>
									{installingDirs.has(`update:${p.name}`) ? (
										<Spinner size={12} />
									) : (
			...
			<DialogContent size="default" className="gap-0">
				...
				<div className="flex flex-col gap-4 p-5">
					<div className="grid grid-cols-3 gap-2 text-[11.5px]">
						<div>
							<div className="text-[var(--tx-faint)]">Author</div>
							<div className="mt-0.5 text-foreground">
								{plugin.author || "Not specified"}
							</div>
						</div>
						<div>
							<div className="text-[var(--tx-faint)]">Version</div>
							<div className="mt-0.5 font-mono text-foreground">
								{plugin.version || "Not specified"}
							</div>
						</div>
						<div>
							<div className="text-[var(--tx-faint)]">Release</div>
							<div className="mt-0.5 text-foreground">
								{plugin.release || "Not specified"}
							</div>
						</div>
					</div>
					{plugin.updateAvailable && (
						<div className="rounded-sm border border-[var(--warn)] bg-[var(--warn-dim)] px-2.5 py-2 text-[11.5px] text-foreground">
							Update available: v{plugin.latestVersion}. Use the Update button
							in the Plugins list to load it.
						</div>
					)}
					<section>
						<h3 className="m-0 text-[11px] font-bold uppercase tracking-wide text-[var(--tx-faint)]">
							Description
						</h3>
						<p className="mt-1.5 mb-0 text-[12px] leading-relaxed text-foreground">
					...
					<section className="border-l-2 border-primary pl-3">
						...
						<p className="mt-1.5 mb-0 text-[12px] leading-relaxed text-foreground">
							{plugin.useCase ||
								"Use this plugin to extend consolio for a recurring API development workflow."}
						</p>
					</section>
					{plugin.homepage && (
						<a
...
/* ── Mock servers ─────────────────────────────────────────────────────────── */
export function _MockManagerModal() {
	const showNotif = useStore((s) => s.showNotif)
	const close = () => useStore.setState({ modal: null })
	...
	const loadMocks = useCallback(async () => {
		setLoading(true)
		try {
			setMocks(await apiFetch("/api/mocks"))
		} catch {}
		setLoading(false)
	}, [])
	...
	useEffect(() => {
		setRoutes(selected?.routes || [])
	}, [selected?.routes]) // eslint-disable-line react-hooks/exhaustive-deps

	const createMock = async () => {
		if (!newName.trim() || !newPort) {
			showNotif("Name and port are required", "error")
			return
		}
		const mock = await apiFetch("/api/mocks", {
			method: "POST",
			body: { name: newName.trim(), port: parseInt(newPort, 10) },
		})
		setNewName("")
		setCreating(false)
		await loadMocks()
	...
	const toggleRunning = async (mock, e) => {
		...
		try {
			await apiFetch(
				`/api/mocks/${mock.id}/${mock.running ? "stop" : "start"}`,
				{ method: "POST" },
			)
			await loadMocks()
		} catch {
			showNotif("Could not toggle mock server", "error")
		}
	}

	const saveRoutes = async () => {
		if (!selected) return
		await apiFetch(`/api/mocks/${selected.id}`, {
			method: "PUT",
			body: { routes },
		})
		await loadMocks()
	...
	const updateRoute = (i, patch) =>
		setRoutes((r) =>
			r.map((route, j) => (j === i ? { ...route, ...patch } : route)),
		)
		...
		>
			<div className="flex min-h-[360px] gap-3.5">
				<div className="flex w-[220px] shrink-0 flex-col gap-1">
					...
					{mocks.map((m) => (
						<div
							...
							tabIndex={0}
							className={cn(
								"flex cursor-pointer items-center gap-2 rounded-sm border border-[var(--bd-faint)] bg-[var(--bg-raised)] px-2 py-1.5",
								m.id === selectedId && "border-primary",
							)}
						...
						>
							...
							<div className="flex min-w-0 flex-1 flex-col">
								...
								<span className="font-mono text-[10px] text-[var(--tx-faint)]">
									:{m.port} · {m.routes?.length || 0} routes
								</span>
							</div>
							<button
								type="button"
								className="flex items-center p-0.5 text-[var(--tx-faint)] transition-colors hover:text-foreground"
								onClick={(e) => toggleRunning(m, e)}
								title={m.running ? "Stop" : "Start"}
							>
					...
					{creating ? (
						<div className="flex flex-col gap-1.5 rounded-sm border border-dashed border-[var(--bd-subtle)] p-2">
							...
							<div className="flex gap-1.5">
								...
								<Btn variant="primary" size="sm" onClick={createMock}>
									Create
								</Btn>
							</div>
						</div>
				...
				<div className="flex min-w-0 flex-1 flex-col gap-2.5 overflow-y-auto">
					{!selected ? (
						<p className="p-6 text-center text-[12px] text-[var(--tx-faint)]">
							Select a mock set to edit its routes
						</p>
					) : (
						<>
							{routes.map((route, i) => (
								<div
									key={route.id}
									className="flex flex-col gap-1.5 rounded-md border border-[var(--bd-faint)] bg-[var(--bg-raised)] p-2"
								>
									...
									<KVTable
										rows={route.headers || []}
										onChange={(v) => updateRoute(i, { headers: v })}
							...
							<div className="flex items-center gap-2">
								<Btn variant="ghost" size="sm" onClick={addRoute}>
									<Icon name="plus" size={11} /> Add Route
								</Btn>
								<div className="flex-1" />
								<Btn variant="primary" size="sm" onClick={saveRoutes}>
									Save Routes
								</Btn>
							</div>
						</>
...
export function _DashboardModal() {
	const collections = useStore((s) => s.collections)
	const modalData = useStore((s) => s.modalData)
	const close = () => useStore.setState({ modal: null })
	...
	const load = useCallback(async () => {
		...
		try {
			...
			const [a, s, m] = await Promise.all([
				...
				collectionId
					? apiFetch(`/api/collections/${collectionId}/score`)
					: Promise.resolve(null),
				collectionId
					? apiFetch(`/api/collections/${collectionId}/mcp-manifest`)
					: Promise.resolve(null),
			])
			setAnalytics(a)
			setScore(s)
	...
	const copyMcpConfig = () => {
		navigator.clipboard.writeText(
			JSON.stringify(mcpManifest.configSnippet, null, 2),
		)
			...
			{!loading && analytics && (
				<>
					<div className="flex flex-wrap gap-2.5">
						...
						<DashStat
							label="P95 latency"
							value={`${analytics.p95LatencyMs}ms`}
						/>
						<DashStat
							label="Error rate"
						...
						<>
							<div className="mt-4 flex items-center justify-between">
								...
								<Btn
									variant="ghost"
									size="sm"
									onClick={() => setShowMcpConfig((v) => !v)}
								>
									<Icon name="sparkle" size={11} />{" "}
									{showMcpConfig ? "Hide" : "Generate"} config
								</Btn>
							</div>
							<p className="my-1 text-[11px] leading-[1.5] text-[var(--tx-faint)]">
								This collection can be served as an MCP server — each request
								becomes a tool an AI agent can call directly.
							</p>
							<div className="flex flex-col gap-1">
								{mcpManifest.tools.map((t) => (
									<div
										key={t.name}
										className="flex cursor-default items-center gap-2 rounded-sm border border-[var(--bd-faint)] bg-[var(--bg-raised)] px-2 py-1.5"
									>
							...
							{showMcpConfig && (
								<div className="mt-2.5 rounded-md border border-[var(--bd-subtle)] bg-[var(--bg-overlay)] px-3.5 py-3">
									...
									<pre className="m-0 overflow-x-auto rounded-sm border border-[var(--bd-faint)] bg-[var(--bg-raised)] px-3 py-2.5 font-mono text-[11px] leading-[1.5] whitespace-pre text-foreground">
										{JSON.stringify(mcpManifest.configSnippet, null, 2)}
									</pre>
									<div className="mt-2 flex justify-end">
										<Btn variant="primary" size="sm" onClick={copyMcpConfig}>
											Copy config
										</Btn>
									</div>
								</div>
					...
					{score?.topIssues?.length > 0 && (
						<>
							<h3 className="m-0 mt-4 text-[11px] font-bold tracking-wide text-[var(--tx-faint)] uppercase">
								Top issues
							</h3>
							<div className="mt-2 flex flex-col gap-1">
								{score.topIssues.map((issue) => (
									...
									>
										...
										<span className="shrink-0 text-[11px] text-[var(--tx-faint)]">
											{issue.count} request
											{issue.count === 1 ? "" : "s"}
										</span>
									</div>
					...
					{analytics.requestBreakdown.length === 0 && (
						<p className="p-6 text-center text-[12px] text-[var(--tx-faint)]">
							No tracked calls yet — requests sent from a saved collection
						...
						{analytics.requestBreakdown.map((r) => (
							...
							>
								<div className="flex min-w-0 flex-1 flex-col">
									...
									<span className="font-mono text-[10px] text-[var(--tx-faint)]">
										{r.totalRequests} calls · avg {r.avgLatencyMs}ms ·{" "}
										{r.errorRate}% errors
									</span>
								</div>
							</div>
...
export function _ProfilesModal() {
	const collections = useStore((s) => s.collections)
	const modalData = useStore((s) => s.modalData)
	const showNotif = useStore((s) => s.showNotif)
	const close = () => useStore.setState({ modal: null })
	...
	const load = useCallback(async () => {
		...
		await Promise.all(
			list.map(async (p) => {
				impacts[p.id] = await apiFetch(
					`/api/collections/${col.id}/profiles/${p.id}/impact`,
				)
			}),
		)
		setImpact(impacts)
	...
	const deleteProfile = async (id) => {
		await apiFetch(`/api/collections/${col.id}/profiles/${id}`, {
			method: "DELETE",
		})
	...
	const exportProfile = async (profile, format) => {
		const filtered = await apiFetch(
			`/api/collections/${col.id}/profiles/${profile.id}/apply`,
		)
		const data =
			format === "postman"
				? exportPostmanCollection(filtered)
				: format === "openapi"
					? exportOpenAPI(filtered)
					: exportInsomniaCollection(filtered)
		downloadJson(
			`${(filtered.name || "collection").replace(/\s+/g, "_")}.${format}.json`,
			data,
		)
	}
	...
	return (
		<Modal
			title={`Audience Profiles — ${col.name}`}
			icon="shield"
			onClose={close}
			wide
			footer={
			...
			{!editing && (
				<>
					<p className="mb-3 text-[11.5px] text-[var(--tx-faint)]">
						A profile scopes this collection to a subset of requests for a given
						audience — export or generate an MCP server from just that slice,
						with chosen headers/params stripped.
					</p>
					<Btn
						variant="primary"
						size="sm"
					...
					<div className="flex flex-col gap-1">
						{profiles.map((p) => (
							<div
								key={p.id}
								className="flex cursor-default items-start gap-2 rounded-sm border border-[var(--bd-faint)] bg-[var(--bg-raised)] px-2 py-1.5"
							>
								<div className="flex min-w-0 flex-1 flex-col">
									<span className="overflow-hidden text-ellipsis whitespace-nowrap text-[12px] text-foreground">
										{p.name}
									...
									<span className="font-mono text-[10px] text-[var(--tx-faint)]">
										{impact[p.id]
											? `${impact[p.id].includedRequests} of ${impact[p.id].totalRequests} requests`
											: "…"}
										{p.redactHeaders.length > 0 &&
											` · redacts ${p.redactHeaders.length} header(s)`}
										{p.redactParams.length > 0 &&
											` · redacts ${p.redactParams.length} param(s)`}
									</span>
								</div>
								<select
									...
									value=""
									onChange={(e) => {
										if (e.target.value) exportProfile(p, e.target.value)
										e.target.value = ""
									}}
...
function ProfileEditor({ collection, profile, onCancel, onSaved }) {
	...
	const [redactHeaders, setRedactHeaders] = useState(
		(profile?.redactHeaders || []).join(", "),
	)
	const [redactParams, setRedactParams] = useState(
		(profile?.redactParams || []).join(", "),
	)
	...
	const save = async () => {
		if (!name.trim()) {
			showNotif("Profile name is required", "error")
			return
		}
		setSaving(true)
		const body = {
		...
		try {
			if (profile)
				await apiFetch(
					`/api/collections/${collection.id}/profiles/${profile.id}`,
					{ method: "PUT", body },
				)
			else
				await apiFetch(`/api/collections/${collection.id}/profiles`, {
					method: "POST",
					body,
				})
			...
			showNotif(e.message || "Failed to save profile", "error")
		}
		setSaving(false)
	}
		...
		<div className="flex flex-col gap-2.5">
			<FormGroup label="Name">
				<Input
					value={name}
					onChange={(e) => setName(e.target.value)}
					placeholder="Public, Partner, AI Agent…"
				/>
			</FormGroup>
			<FormGroup label="Description (optional)">
				<Input
					...
					placeholder="What is this audience allowed to see?"
				/>
			</FormGroup>
			<FormGroup label="Mode">
				<Select value={mode} onChange={(e) => setMode(e.target.value)}>
					<option value="allowlist">
						Allowlist — only checked requests are included
					</option>
					<option value="blocklist">
						Blocklist — everything except checked requests
					</option>
				</Select>
			</FormGroup>
			<FormGroup
				label={mode === "allowlist" ? "Included requests" : "Excluded requests"}
			>
				<div className="max-h-[180px] overflow-y-auto rounded-md border border-[var(--bd-subtle)] bg-[var(--bg-overlay)] px-3.5 py-3">
					...
					{(collection.requests || []).length === 0 && (
						<span className="text-[11px] text-[var(--tx-faint)]">
			...
			<FormGroup label="Redact headers (comma-separated, optional)">
				<Input
					value={redactHeaders}
					onChange={(e) => setRedactHeaders(e.target.value)}
					placeholder="X-Admin-Token, X-Internal-Id"
				/>
			</FormGroup>
			<FormGroup label="Redact query params (comma-separated, optional)">
				<Input
					value={redactParams}
					onChange={(e) => setRedactParams(e.target.value)}
					placeholder="debug, internal_flag"
				/>
			</FormGroup>
			<div className="mt-1 flex justify-end gap-2">
				...
				<Btn variant="primary" onClick={save} disabled={saving}>
					{saving ? <Spinner size={12} /> : "Save profile"}
				</Btn>
			</div>
		</div>
	)
